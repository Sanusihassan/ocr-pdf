...
import { OCRPDFHOWTO_hi } from "@/src/how-to";
...
export default ({ item, lang, initialPremiumStatus }: { item: _tool["Ocr_PDF"]; lang: string; initialPremiumStatus: boolean }) => {
    ...
    <>
      <Head>
        ...
        <script
          type="application/ld+json"
          dangerouslySetInnerHTML={{
            __html: JSON.stringify(OCRPDFHOWTO_hi),
          }}
        />
      ...
      <div className="container">
        <HowTo howTo={OCRPDFHOWTO_hi} alt={item.seoTitle} imgSrc={item.to.replace("/", "")} />
      </div>
      <Footer lang={lang} title={item.seoTitle} />
    </>
  );
};
